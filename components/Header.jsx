...
const MENU = [
  ...
  },
  {
    ...
    type: 'single',
    items: [
      ['MBA in Information Technology', '/courses?c=online-mba&s=information-technology'],
      ['MBA in Business Analytics', '/courses?c=online-mba&s=business-analytics'],
      ['MBA in Hospital Administration Management', '/courses?c=online-mba&s=hospital-administration-management'],
      ['MBA in International Trade Management', '/courses?c=online-mba&s=international-trade-management'],
      ...
      ['MBA in Business Management', '/courses?c=online-mba&s=business-management'],
      ['MBA in Project Management', '/courses?c=online-mba&s=project-management'],
      ['MBA in Marketing Management', '/courses?c=online-mba&s=marketing-management'],
    ],
  },
  {
    label: 'Student Zone',
    ...
    items: [
      ...
      ['DUSOL Alternative Universities', '/student-zone?p=dusol-alternative-universities'],
    ],
  },
      ...
      <div className="dropdown cols">
        {item.groups.map((g, gi) => (
          <div key={gi}>
            ...
            {g.items.map((it, ii) => (
              <Link key={ii} className="dd-link" href={it[1]}>{it[0]}</Link>
            ))}
          </div>
        ))}
      </div>
      ...
      <>
        {item.groups.map((g, gi) => (
          <div key={gi}>
            ...
            {g.items.map((it, ii) => (
              <Link key={ii} href={it[1]} onClick={onClose}>{it[0]}</Link>
            ))}
          </div>
        ))}
      </>
...
export default function Header() {
  ...
  const closeMobile = () => { setMobileOpen(false); setOpenItem(null) }
  ...
  return (
    <>
          ...
          <nav className="nav">
            <Link className="logo" href="/">
              ...
              <span className="logo-text">
                <strong style={{ fontFamily: 'var(--font-head)', fontWeight: 800, fontSize: '20px', color: 'var(--purple-dark)', letterSpacing: '.5px' }}>DU SOL</strong>
      ...
      <aside className={`mobile-menu${mobileOpen ? ' open' : ''}`}>
        <button className="mm-close" onClick={closeMobile}>&times;</button>
        <div className="mm-logo">DU SOL</div>
        {MENU.map((m, i) => (
          <div key={i} className={`mm-item${openItem === i ? ' open' : ''}`}>
            {'link' in m ? (
              <div className="mm-link">
                <Link href={m.link} style={{ color: 'inherit' }} onClick={closeMobile}>
                  {m.label}
                </Link>
              </div>
            ) : (
        ...
        <Link href="/blogs" className="btn btn-gold btn-block" style={{ marginTop: '18px' }} onClick={closeMobile}>
          ENQUIRY
        </Link>
      </aside>
    </>
